class SettingsController < ApplicationController
  ...
  def export_to_csv  
    ...
    csv_string = Settings.generate_cvs_string(@accepted_users_docs, @accepted_users_art)
    # CSV.generate do |csv|
  ...
  def export_rejected_to_csv  
    require 'csv'     
    @rejected_users_docs = Submitter.joins(:documents).where("accepted = false").select("first_name", "last_name", "bio", "school", "teacher", "email", "documents.title", "documents.style")
    @rejected_users_art = Submitter.joins(:artworks).where("accepted = false").select("first_name", "last_name", "bio", "school", "teacher", "email", "artworks.title")
    csv_string = Settings.generate_cvs_string(@rejected_users_docs, @rejected_users_art)
